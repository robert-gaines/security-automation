...
def GetContainers(key):
    ...
    if(req.status_code == 200):
        ...
        for entry in data:
            ...
            name        = entry['name']
            if(status != 'closed' and (ctr_id not in ctr_ids)):
                CreateCase(key,ctr_id,name,description)
...
def CreateCase(key,ctr_id,name,description):
    workbook = "1"
    ...
    if("[EDR]" in name):
        workbook = "8"
    if("Active" in name or "VMWare" in name):
        workbook = "3"
